...
public class EnumButton
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        RenderUtil.rect(context, this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, activeColor(this.x, this.y, this.isHovering(mouseX, mouseY) ? ClickGuiModule.getInstance().topColor.getValue().getAlpha() : ClickGuiModule.getInstance().color.getValue().getAlpha()));
